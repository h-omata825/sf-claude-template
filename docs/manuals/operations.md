...
cp .claude-template-tmp/.mcp.json.example .mcp.json.example
cp .claude-template-tmp/upgrade.sh upgrade.sh
...
2. 差分をチェックして変更内容を表示
3. 確認後、`.claude/` 配下を上書き（`settings.json` は除外）
4. コミット・PR作成の手順を案内

#### 方法B: 手動コピー（upgrade.shが使えない場合）
...
# 5. コミット
git add .claude/ upgrade.sh .mcp.json.example
...
- `.mcp.json` は `.gitignore` 対象のため、Git管理されない（トークンは安全）
- `.mcp.json.example`（トークンなし）のみがGit管理される
- 各メンバーは `/setup-mcp` で自分のトークンを設定する
- 後からMCPを追加・変更したい場合は `/setup-mcp` を再実行
...
- [ ] `docs/` の資材を最新化
- [ ] テンプレートの新バージョンがないか確認（`bash upgrade.sh` で確認可能）
