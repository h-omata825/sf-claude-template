...
├── .mcp.json                    ← 【.gitignore対象】トークン入りMCP設定（/setup-mcpで生成）
...
| `.mcp.json` | **◯ /setup-mcpで生成** | トークン入りMCP設定（.gitignore対象・Git管理外） |
| `.mcp.json.example` | **✕ 触らない** | MCPテンプレート。テンプレートアップグレード時に上書きされる |
| `docs/` 配下 | **◯ 自由に追加** | プロジェクト資材の蓄積 |
| `upgrade.sh` | **✕ 触らない** | テンプレートアップグレード時に自身も更新される |
...
処理:
1. 使用するMCPを選択（GitHub / Slack / Notion / Google Drive / Playwright）
2. 各MCPのトークンを入力
3. `.mcp.json.example` をもとに `.mcp.json` を生成（トークン入り）

`.mcp.json` は `.gitignore` 対象のため、トークンがGitにpushされる心配はない。
後からMCPを追加・変更したい場合は `/setup-mcp` を再実行する。
...
| ファイル | 役割 | Git管理 |
|---|---|---|
| `.mcp.json.example` | MCPテンプレート（トークンなし） | **される**（共有用） |
| `.mcp.json` | 実際の設定（トークン入り） | **されない**（.gitignore対象） |
...
#### 方法1: /setup-mcp コマンド（推奨）

```
/setup-mcp
```

対話形式でMCPを選択し、トークンを入力するだけで `.mcp.json` が生成される。
...
- `.mcp.json` は `.gitignore` に含まれるため、トークンがGitHubにpushされることはない
- `.mcp.json.example`（トークンなし）のみがGit管理される
- メンバーは各自で `/setup-mcp` を実行してトークンを設定する
...
2. テンプレートファイル（`.claude/`, `CLAUDE.md`, `docs/`, `.mcp.json.example`, `upgrade.sh`）をコピー
3. `.gitignore` に `.claude/settings.json` と `.mcp.json` を追加
...
### upgrade.sh（テンプレートアップグレード）

テンプレートに更新があった場合、プロジェクトフォルダで実行:

```bash
bash upgrade.sh
# or（テンプレートURLやバージョンを指定）
bash upgrade.sh https://github.com/xxx/template.git v1.1.0
```
...
3. 差分サマリを表示して確認を求める
4. 適用後、コミット・PR作成の手順を案内

更新対象 / 対象外:
| 対象 | 更新 |
|---|---|
| `.claude/CLAUDE.md`, `agents/`, `commands/` | ✅ 上書き |
| `.mcp.json.example`, `upgrade.sh` | ✅ 上書き |
| `.claude/settings.json` | ❌ 除外（個人設定） |
| `CLAUDE.md`（ルート）, `docs/`, `force-app/` | ❌ 除外（プロジェクト固有） |
