...
| コマンド | 実行エージェント | 概要 |
|---|---|---|
| `/sf-memory` | salesforce-architect | Claude Codeの記憶形成。組織概要・オブジェクト構成・マスタデータ・設計を会話形式で選択して実行 |
| `/sf-retrieve [対象]` | salesforce-dev | package.xmlを生成してメタデータを取得（指定/標準セット/全量の3モード） |
...
Claude Codeが正確に動作するための記憶（docs/）を構築・更新する。会話形式でカテゴリを選択して実行する。

**カテゴリ:**
| カテゴリ | 生成先 | 内容 |
|---|---|---|
| 組織概要・環境情報 | `docs/overview/`, `docs/requirements/` | 組織プロフィール・要件定義書 |
| オブジェクト・項目構成 | `docs/catalog/` | オブジェクト・項目定義書 |
| マスタデータ・自動化 | `docs/data/` | マスタデータ・メールテンプレート・レポート・承認プロセス等 |
| 設計・機能仕様 | `docs/design/` | 機能別設計書（逆引き生成・既存資料インポートにも対応） |

**実行パターン:**
- **初回セットアップ**: 「全て」を選択 → 4カテゴリをサブエージェントで並列処理 → 2周目で横断補完
- **更新**: 変更があったカテゴリを選択 → 対象を絞り込んで（オブジェクト名・機能名等）再実行
- **資料の取り込み**: ファイルパス/フォルダパスを指定すると `.xlsx/.docx/.pdf` を自動変換して統合
