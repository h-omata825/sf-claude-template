...
ST_META_1 = [
    ("プロジェクト名", 2, 5,   6, 18, "=サマリー!G3"),
    ("作成日",         19, 22, 23, 31, "=サマリー!W3"),
]
ST_META_2 = [
    ("作成者",  2, 5,   6, 18, "=サマリー!G4"),
    ("件数",   19, 22, 23, 31, ""),  # 件数は generate 側で上書き
]
...
def build_sheet_template(wb):
    ...
    set_h(ws, ST_META_ROW_1, 22)
    for label, ls, le, vs, ve, formula in ST_META_1:
        MW(ws, ST_META_ROW_1, ls, le, label,
           bold=True, bg=C_LABEL_BG, border=B_all(), h="center")
        MW(ws, ST_META_ROW_1, vs, ve, formula, border=B_all())
    set_h(ws, ST_META_ROW_2, 22)
    for label, ls, le, vs, ve, formula in ST_META_2:
        MW(ws, ST_META_ROW_2, ls, le, label,
           bold=True, bg=C_LABEL_BG, border=B_all(), h="center")
        MW(ws, ST_META_ROW_2, vs, ve, formula, border=B_all())
