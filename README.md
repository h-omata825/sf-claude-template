...
| コマンド | 内容 |
|---|---|
| `/sync` | セッション開始時に実行。最新版取得・テンプレートバージョン確認 |
| `/setup-sf-project` | Salesforce組織の認証（クリック選択式） |
| `/setup-mcp` | MCP連携の設定（Backlog・GitHub・Slack等） |
| `/sf-retrieve [対象]` | package.xml 生成・メタデータ取得（standard / all / 個別指定） |
| `/sf-analyze [資料]` | 組織解析→組織プロフィール・要件定義書を自動生成。2回目以降は全資料を統合して精緻化 |
| `/sf-catalog [オブジェクト名]` | オブジェクト・項目定義書の自動生成（引数なし=全量） |
| `/sf-design [内容\|逆引き]` | 機能別設計書の作成・既存コードからの逆引き生成（保守案件向け） |
| `/sf-data [内容]` | マスタデータ・メールテンプレート・レポート・データ統計・品質情報の収集と資料化 |
| `/backlog [課題ID]` | Backlog課題を読み込み、影響範囲分析・対応方針をメリデメ付きで提案 |
| `/git-pr` | ブランチ作成・コミット・プッシュ・PR作成 |
| `/upgrade [タグ]` | テンプレート最新版を取得して `.claude/` を更新・自動コミット・push |
