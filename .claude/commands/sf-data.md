...
まず `docs/data/` 配下にmdファイルが存在するか確認する。

### 存在しない → **初回生成モード**

組織からデータを収集して資料を新規作成する。以下を読み込んでから Phase 1 へ進む:
...
**初回は組織からデータを収集して資料を作る。2回目以降は以下の3つを統合してより正確・詳細な資料に育てる:**

| ソース | 使い方 |
|---|---|
| 組織データ（再収集） | データの変化（追加・変更・削除）を検出 |
| 既存の資料 | 手動追記・確定済み情報・補足メモを保持 |
| 現在のセッション情報 | 会話で確認したデータの意味・業務上の使われ方・移行判断を反映 |

**手順:**
1. `docs/data/` 配下の全ファイルを読み込む
2. 上記コンテキストファイルを読み込む
3. 現在のセッションで確認・判明した情報を整理する（データの意味、クレンジングルール、移行方針等）
4. 組織からデータを再収集して差分を検出する
5. 3つのソースを統合して各資料を更新する
6. **バージョン番号をインクリメントする**（v1.0 → v1.1）
7. `docs/changelog.md` に変更サマリを追記する

**重要ルール:**
- 手動追記・業務上のメモは絶対に消さない
- セッションで確定したデータルールは「推定」→「確定」に昇格させる
- データ件数の大きな変動は必ず報告する
...
資料の作成・更新が完了したら、ルートの `CLAUDE.md` を読み込み、以下を更新する。
**手動記入済みの内容は上書きしない。空欄・プレースホルダーのみ更新する。**

- **主要カスタムオブジェクト**セクションに、マスタデータで扱ったオブジェクトを列挙（なければセクションを追加）
- **注意事項・地雷**セクションに、データ品質チェックで検出した問題（空欄率が高い重要項目・重複の多いオブジェクト等）を追記候補として提示し、ユーザーに確認してから追記
