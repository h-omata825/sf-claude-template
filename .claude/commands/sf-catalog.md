...
まず `docs/catalog/` 配下にmdファイルが存在するか確認する。

### 存在しない → **初回生成モード**

組織メタデータから定義書を新規作成する。Phase 1 へ進む。

参照コンテキスト（あれば読み込む）:
| ファイル | パス |
|---|---|
| 組織プロフィール | `docs/overview/org-profile.md` |
| 要件定義書 | `docs/requirements/requirements.md` |

### 存在する → **アップデートモード**

**初回は組織メタデータから骨格を作る。2回目以降は以下の3つを統合してより正確・詳細な定義書に育てる:**

| ソース | 使い方 |
|---|---|
| 組織メタデータ（再収集） | 項目の追加・変更・削除を検出 |
| 既存の定義書 | 手動追記・確定済み情報を保持 |
| 現在のセッション情報 | 会話で判明した業務ルール・用語の正確な意味・確定事項を反映 |

**手順:**
1. `docs/catalog/` 配下の全ファイルを読み込む
2. 上記コンテキストファイルが存在する場合も読み込む
3. 現在のセッションで確認・判明した情報を整理する（項目の業務上の意味、使われ方、ピックリスト値の意味等）
4. 組織メタデータを再収集して差分を検出する
5. 3つのソースを統合して各定義書を更新する
