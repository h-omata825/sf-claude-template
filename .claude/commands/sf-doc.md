...
python "{カレントディレクトリ}/scripts/python/sf-doc-mcp/generate_basic_doc.py" \
  ...
  --output "{カレントディレクトリ}/01_基本設計/プロジェクト概要書.xlsx" \
  --author "{作成者名}"
```

完了後、出力パスを表示:
- `01_基本設計/プロジェクト概要書.xlsx`
...
`01_基本設計/` 内の `オブジェクト項目定義書_v*.xlsx` を確認する:
...
`01_基本設計/` 内の `機能一覧_v*.xlsx` を確認:
```bash
python -c "
import pathlib, glob
files = sorted(glob.glob('01_基本設計/機能一覧_v*.xlsx'))
for f in files:
    print(f)
"
```
...
python "{カレントディレクトリ}/scripts/python/sf-doc-mcp/scan_features.py" \
  ...
  --output "{カレントディレクトリ}/01_基本設計/.features_tmp.json"
```
...
生成完了後、一時ファイルを削除:
```bash
python -c "import pathlib; p = pathlib.Path('{カレントディレクトリ}/01_基本設計/.features_tmp.json'); p.unlink(missing_ok=True)"
```
