...
| 機能一覧 | `force-app/`（Apex/Flow/LWC 等を直接スキャン）<br>`docs/.sf/feature_ids.yml`（ID台帳） | `/sf-retrieve` |
...
- `docs/.sf/feature_ids.yml`（機能ID台帳 — 初回実行時に自動生成）
...
`{output_dir}/01_基本設計/` 内の `機能一覧_v*.xlsx` を確認:
```bash
python -c "
import pathlib, glob, os
files = sorted(glob.glob(r'{output_dir}/01_基本設計/機能一覧_v*.xlsx'), key=os.path.getmtime, reverse=True)
for f in files:
    print(f)
"
```
...
```bash
# feature_ids.yml は docs/.sf/feature_ids.yml を自動参照する（スクリプト内部で解決）。
# 初回実行時はスクリプトが自動生成する。手動で用意する必要はない。
python "{project_dir}/scripts/python/sf-doc-mcp/scan_features.py" \
  --project-dir "{project_dir}" \
  --output "{output_dir}/01_基本設計/.features_tmp.json"
```

スキャン完了後、機能一覧を生成:
```bash
python "{project_dir}/scripts/python/sf-doc-mcp/generate_feature_list.py" \
  --input "{output_dir}/01_基本設計/.features_tmp.json" \
  --output-dir "{output_dir}/01_基本設計" \
  --author "{作成者名}" \
  --project-name "{プロジェクト名}" \
  --source-file "{output_dir}/01_基本設計/{既存ファイル名（新規は省略）}" \
  --version-increment {minor または major}
```

生成完了後、一時ファイルを削除:
```bash
python -c "import pathlib; p = pathlib.Path(r'{output_dir}/01_基本設計/.features_tmp.json'); p.unlink(missing_ok=True)"
```
