...
**テンプレート置換ルール（厳守）:**
- Python インラインコード内の `{project_dir}` `{output_dir}` `{author}` `{version_increment}` `{latest_obj_file}` 等の `{...}` は f-string ではなく **Claude が実行前に実値でテキスト置換する** プレースホルダー。Bash に渡す前に、値の種別に応じて以下の規則で置換する:
  - **パス値** (`{project_dir}` / `{output_dir}` / `{latest_obj_file}` 等): Windows パスの `\` はすべて `/` に置換し、末尾の `/` は除去する（例: `C:\work\` → `C:/work`）。raw string 末尾 `\` による SyntaxError を回避するため。`pathlib.Path` は Windows でも forward slash を正しく解釈する。
  - **任意文字列値** (`{author}` / `{作成者名}` / `{システム名称}` 等): シングルクォートで囲まれた箇所 (`'{author}'`) への埋め込み時は、値内の `'` を `\'` にエスケープし、改行 (`\n` `\r`) は空白に置換する（例: `O'Brien` → `O\'Brien`）。シェル引数 (`"{author}"`) への埋め込み時は値内の `"` を `\"` にエスケープする。
  - **列挙値** (`{version_increment}`): `minor` / `patch` / `major` 以外が指定された場合は `minor` にフォールバックし、ユーザーに「未知の値のためminorに置換」と警告する。
- 上記は `scripts/python/sf-doc-mcp/` 配下スクリプトや連鎖的に呼び出すコマンドでも同じ規則で適用すること。
...
**複数選択時の実行順序（この順番に従うこと）:**

```
Step A（プロジェクト概要書）→ Step B（オブジェクト定義書）
```

「プロジェクト概要書」のみ選択 → Step A のみ実行して終了。
「オブジェクト定義書」のみ選択 → Step B のみ実行して終了。
両方選択 → Step A → Step B の順に実行。
...
## Step 0-3: 事前確認（両方選択時のみ・ここで全質問を終わらせる）

> **両方選択した場合はこのセクションを実行する。** Step A・B で必要な情報を一括収集し、以降は一切ユーザーへの確認を行わない。

> ⚠️ **drift 防止（必読）**: 以下の B-1〜B-4 は **単独実行時の B-1〜B-4（後方の Step B セクション）と同じロジック** を事前一括実行しているもの。どちらか一方のロジックを修正したら、必ずもう一方も同じ変更を入れること。特に `SF_ALIAS` の取り出し・`（前回/自動取得）` の除去・`latest_obj_file._meta` 読込 は両方で同じ挙動でなければならない（過去に `latest_obj_file._meta` 読込が Step 0-3 側だけ欠落して更新版のオブジェクトリストが壊れる C-3 が発生）。
...
> **重要**: `システム名称` として保持する値は、label から `（前回/自動取得）` を除去した **元の値だけ**。ラベルの付記文字列は UI 表示用であり、資料（xlsx の表紙・_meta シート）には含めない。Step B-3 単独実行時と同じ取り扱いとする。

**B-4: 対象オブジェクトの選択**

**新規の場合** (`version_increment = minor` かつ `latest_obj_file` が空) は `docs/catalog/_index.md` からオブジェクト一覧を取得:
```bash
python -c "
import re, pathlib
...
**更新の場合** (`latest_obj_file` が取得できている) は既存ファイルの `_meta` から前回オブジェクトを取得する（単独実行時 B-4 と同じロジック。**C-3 の再発防止のため、ここで必ず `latest_obj_file` を読むこと**）:
```bash
python -c "
import sys
...
AskUserQuestion で提示:
- 新規の場合: label: "_index.md の全オブジェクト（{n}件）"、description: "最終 /sf-memory 時点の使用中オブジェクト（標準→カスタム順）"
- 更新の場合: label: "既存と同じ（{n}件）"、description: "前回と同じオブジェクトで再生成" / label: "既存＋追加"、description: "テキストで追加するオブジェクトを入力"

`{n}` はそれぞれの Python スクリプトが `COUNT:` で出力した件数を用いて実行時に埋める。Other の場合はチャットで入力してもらう。結果を `オブジェクトリスト` として保持（新規は `_index.md` 全件、「既存と同じ」は前回オブジェクトそのまま、「既存＋追加」は結合）。
...
**両方選択時**: Step 0-3 で確認済み。スキップして A-1 へ進む。
...
**両方選択時**: Step 0-3 で設定済み（`SF_ALIAS` / `version_increment` / `システム名称` / `オブジェクトリスト`）。B-1〜B-4 をスキップして B-5 へ進む。

**単独実行の場合**: AskUserQuestion で確認:
- label: "このまま続ける"
- label: "/sf-memory カテゴリ2 を実行してから続ける（終了）"

> ⚠️ **drift 防止（必読）**: 以下 B-1〜B-4 と、前方 `Step 0-3 の「Step B 事前設定」` は **同じロジックを 2 箇所に書いているコピーペア**。どちらか一方を修正する際は、必ずもう一方にも同じ修正を反映すること。整合性がズレると両方選択時と単独実行時で挙動が変わる（例: 更新モードのオブジェクト一覧取得）。
...
取得した一覧をもとに AskUserQuestion で提示（Other は自動表示。`{n}` は直前のスクリプトが出力した `COUNT:` の値で置換）:
...
**両方選択時**: Step 0-3 で確認済み。確認なしでそのまま生成を開始する。
