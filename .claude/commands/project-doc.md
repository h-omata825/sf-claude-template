...
- **概要書**             → Step A（下記）へ
- **データモデル定義書**     → Step D（下記）へ
- **オブジェクト定義書** → Step 1（既存フロー）へ
- **機能別設計書**       → Step C（下記）へ

---

## Step A: プロジェクト概要書 ＋ 業務フロー図の生成

### A-1: docs/ フォルダの確認

カレントディレクトリに必要なファイルが存在するか確認:
```bash
python -c "
import pathlib
...
diagrams = docs / 'overview'      / 'diagrams.json'
swimlanes = docs / 'flow'         / 'swimlanes.json'
print('profile:', profile.exists())
print('req:', req.exists())
print('diagrams:', diagrams.exists())
print('swimlanes:', swimlanes.exists())
"
```

- `org-profile.md` / `requirements.md` が両方ない場合:「先に `/sf-memory` を実行してください。」と伝えて終了。
- `diagrams.json` がない場合: 「`docs/overview/diagrams.json` がないため概要書の図スライドはスキップされます。」と表示して続行。
- `swimlanes.json` がない → requirements.md の Mermaid から業務フロー図を生成（標準モード）と表示。

### A-2: 作成者名

AskUserQuestion で提示（1択のみ）:
...
AskUserQuestion で確認後、Bash で順番に実行:
...
完了後、出力ファイル2件のパスを表示して終了:
- `{出力フォルダ}/プロジェクト概要書.pptx`
- `{出力フォルダ}/業務フロー図.pptx`

> **新規プロジェクトで各定義ファイルを作成する場合:**
> - `docs/overview/diagrams.json` — AS-IS/TO-BE/システム構成の図要素定義
> - `docs/flow/swimlanes.json` — スイムレーン形式の業務フロー定義（高品質モード）
> これらがない場合はテキストのみのスライドで生成される。
...
作成者名は AskUserQuestion で提示（1択のみ）:
- label: "スキップ"、description: "作成者名なし"
