---
description: "ClaudeCodeの記憶形成コマンド。Salesforce組織・プロジェクトの情報を収集し、docs/ 配下にClaudeが正確に動作するための記憶を構築する。組織概要・オブジェクト構成・マスタデータ・設計の4カテゴリを会話形式で選択して実行する。"
---

salesforce-architectエージェントとして、Salesforce組織・プロジェクトの情報を収集し、Claude Codeの記憶（docs/）を構築・更新してください。
...
```
1周目: カテゴリ 1 → 2 → 3 → 4 を順番に実行
  （各カテゴリの Phase を全て実行し docs/ を生成）

2周目: 生成した全 docs/ を参照しながら横断的に補完
  - 用語の統一（カテゴリ間で表記ゆれがあれば修正）
  - カテゴリ間の矛盾・不整合の解消
  - 各カテゴリで「要確認」となった事項を他カテゴリの情報で補完できないか確認
  - org-profile.md の用語集・ステークホルダーマップを各定義書・設計書に反映
```

### カテゴリ指定時

選択されたカテゴリのみ実行する。他のカテゴリの既存 docs/ は参照コンテキストとして読み込む。
