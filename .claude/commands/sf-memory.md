...
- オブジェクト・項目構成
- マスタデータ・ワークフロー設定
- 設計・機能仕様（コンポーネント単位の設計書 docs/design/ 生成）
- 機能グループ定義（業務単位FGの推論・docs/_src/feature_groups.yml 生成）
...
Step 2: カテゴリ2・3・4・5 を並列でエージェントへ委譲
  ※ カテゴリ1完了後に実行（org-profile.md を参照するため）
  sf-org-analyst（カテゴリ2: オブジェクト・項目構成）      ┐
  sf-org-analyst（カテゴリ3: マスタデータ・ワークフロー設定）│
  sf-org-analyst（カテゴリ4: 設計・機能仕様）              ├ 並列実行
  sf-org-analyst（カテゴリ5: 機能グループ定義）            ┘
    → 各カテゴリの docs/ を生成
    → 完了サマリを返す
...
✅ カテゴリ5 完了 — docs/_src/feature_groups.yml を生成しました（FG-XXX 件の機能グループ）
🔄 2周目（横断補完）を実行中...
```
...
- 実行するカテゴリ番号・カテゴリ名（エージェント定義は Agent ツールが自動でロードするため全文転記不要）
