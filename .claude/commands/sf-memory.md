...
Phase 3b: 機能グループ定義をエージェントへ委譲（順次）
  ...
  ※ cat3 の完了は待たない（cat5 は cat4 出力にのみ依存）
...
> - 全カテゴリ選択時の順序（Phase 1: cat1 → Phase 2: cat2 → Phase 3: cat3+cat4 並列 → Phase 3b: cat5 → Phase 4: 横断補完）から、**選択されたカテゴリのみを抽出して同じ順序で実行**する。
...
**cat5（機能グループ定義）への追加情報**（cat4 完了後に起動する場合に追加で渡す）:
