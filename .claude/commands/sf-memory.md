---
description: "ClaudeCodeの記憶形成コマンド。Salesforce組織・プロジェクトの情報を収集し、docs/ 配下にClaudeが正確に動作するための記憶を構築する。組織概要・オブジェクト構成・マスタデータ・設計の4カテゴリを会話形式で選択して実行する。全て選択時はカテゴリごとにサブエージェントへ委譲してコンテキスト効率を最大化する。"
---
...
1〜2カテゴリの更新はサブエージェントを使わず直接実行する（オーバーヘッドが不要なため）。
他のカテゴリの既存 docs/ は参照コンテキストとして読み込む。
