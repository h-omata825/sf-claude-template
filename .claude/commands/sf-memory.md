...
> **前提**: このコマンドは Salesforce プロジェクトルート（`docs/` `force-app/` があるフォルダ）を VSCode で開いた状態で実行されることを想定している。Bash ツールの `pwd` は **Git Bash または WSL 環境前提**（forward slash でパスを返す）。

---

AskUserQuestion ツールで以下を **multiSelect: true** で表示する。

**質問**: 「実行するカテゴリを選択してください（複数選択可）」
...
> - **資料確認質問（「読み込ませたい資料がある場合」セクション）は全選択パターンで常に実施する**（全4件選択時もスキップしない）
...
Phase 3: カテゴリ3・4 を並列でエージェントへ委譲
  ...
  ※ 並列実行は「1つの assistant メッセージ内で Agent ツールを2回同時に呼び出す」ことで実現する
  sf-analyst-cat3（カテゴリ3: マスタデータ・ワークフロー設定）┐ 並列実行
  sf-analyst-cat4（カテゴリ4: 設計・機能グループ定義）         ┘
    → 各カテゴリの docs/ を生成
      - cat3: docs/data/ を生成
      - cat4: docs/design/ を生成後、内部で sf-analyst-cat5 に委譲して docs/.sf/feature_groups.yml を生成
    → 完了サマリを返す
...
| 設計・機能グループ定義 | `sf-analyst-cat4`（設計書生成後、内部で `sf-analyst-cat5` に委譲して feature_groups.yml を生成） | cat1 + cat2（catalog/ 必須） |

> **前提チェック**:
> - cat2・cat3・cat4 を選択した場合、実行前に `docs/overview/org-profile.md` の存在を確認する。
> - さらに cat3・cat4 を選択した場合は `docs/catalog/_index.md` の存在も確認する。
...
> - 全カテゴリ選択時の順序（Phase 1: cat1 → Phase 2: cat2 → Phase 3: cat3+cat4 並列 → Phase 4: 横断補完）から、**選択されたカテゴリのみを抽出して同じ順序で実行**する。
> - cat1 と cat2 は依存先のため、選択されている場合は最初に順次実行する。
> - cat3 と cat4 は互いに独立しているため、両方選択された場合は並列実行する（1メッセージ内で Agent ツールを2回同時呼び出し）。cat3 のみ・cat4 のみの場合は単独実行。
> - 単一カテゴリ選択時は Phase 1 のみ（Phase 4 はスキップ）。
> - 例: cat1+cat3 → cat1 実行後に cat3 を単独実行 → Phase 4
> - 例: cat3+cat4 → cat3 と cat4 を並列実行 → Phase 4
> - 例: cat1+cat2+cat4 → cat1 → cat2 → cat4 → Phase 4
