...
**各エージェントの内部構造**: 全エージェントは Step 0b でフェーズ用 `_index-phase{N}.md` + `_index-cross.md` を読んでオプション判定を行う（[à la carte 仕組み](../templates/backlog/_README.md)）。`backlog-implementer` / `backlog-tester` / `backlog-releaser` / `backlog-validator` はさらに Step 0a で `sf-context-loader` を呼び出す。`backlog-investigator` / `backlog-planner` は docs/ を直接全件読みするため Step 0a を持たない。
