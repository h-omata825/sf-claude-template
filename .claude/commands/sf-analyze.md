...
2. Phase 1 で組織情報を再収集する
3. **既存の内容と新しい情報を比較**し、以下を検出する:
   - 新規追加されたもの（オブジェクト、項目、Apex、フロー、ユーザー等）
   - 削除されたもの
   - 変更されたもの（レコード件数の増減、APIバージョン変更等）
   - 前回「要確認」だった事項で、組織に反映されたもの
4. 既存ファイルを更新する（既存の記述は尊重し、差分のみ反映）
5. **変更サマリを `docs/changelog.md` に追記する**

### 差分更新時の重要ルール
- **人が追記・修正した内容は保持する**: 自動生成のテンプレート部分だけでなく、手動で追記された情報（ヒアリング結果、確認済み事項、補足メモ等）は絶対に消さない
- **要件番号（FR-XXX, NFR-XXX）は既存のものを維持**: 番号の振り直しはしない。新規追加分は続番で採番
- **推定→確定の反映**: 前回「推定」「要確認」としていた項目が、手動修正で確定情報になっていた場合はそのまま維持
- **バージョン番号を更新**: 要件定義書のバージョンを v1.0 → v1.1 → v1.2 のように更新
