...
詳細設計＋プログラム設計の両方を選択した場合は **詳細設計 → プログラム設計 → 機能一覧** の順に生成する（詳細設計 JSON をプログラム設計が参照して精度を高めるため）。
「機能一覧」を単独で選択した場合は機能一覧.xlsx を再生成する。
...
```bash
python -c "import pathlib; pathlib.Path('docs/.sf/.author_tmp').write_text('{author}', encoding='utf-8')"
```
...
```bash
python -c "import pathlib; pathlib.Path('docs/.sf/.output_dir_tmp').write_text(r'{output_dir}', encoding='utf-8')"
```
...
## Step 0-3: 事前確認（詳細設計＋プログラム設計の両方を選択した場合のみ・ここで全質問を終わらせる）

> **詳細設計＋プログラム設計の両方を選択した場合はこのセクションを実行する。** 詳細・プログラム設計で必要な対象を一括で決定し、以降の Step では一切ユーザーへの確認を行わない。
> **「機能一覧」を単独で選択した場合はこのセクションをスキップして Step 3 へ進む。**
...
## Step 3: 機能一覧（「機能一覧」単独選択時、またはプログラム設計完了後）

「機能一覧」を選択した場合は Step 0-2 の共通情報取得後に以下を実行する。
プログラム設計と組み合わせた場合は Step 2 完了後に自動で実行される。
