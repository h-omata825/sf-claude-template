...
| 選択 | 実行フロー |
|---|---|
| 詳細設計のみ | Step 0-2 → Step 0-3 不要 → Step 1 |
| プログラム設計のみ | Step 0-2 → Step 0-3 不要 → Step 2 |
| 機能一覧のみ | Step 0-2 → Step 3 |
| 詳細+プログラム設計 | Step 0-2 → Step 0-3 → Step 1 → Step 2 |
| 詳細設計+機能一覧 | Step 0-2 → Step 0-3 不要 → Step 1 → Step 3 |
| プログラム設計+機能一覧 | Step 0-2 → Step 0-3 不要 → Step 2（機能一覧も Step 2 で生成） → Step 3 はスキップ |
| 詳細+プログラム+機能一覧 | Step 0-2 → Step 0-3 → Step 1 → Step 2（機能一覧も Step 2 で生成） |

> **「プログラム設計+機能一覧」または「詳細+プログラム+機能一覧」選択時**: Step 2 の sf-design-writer が機能一覧.xlsx を生成するため、Step 3 は実行しない（二重実行防止）。
...
> **詳細設計 AND プログラム設計が両方選択されている場合はこのセクションを実行する（機能一覧の選択有無は問わない）。** 詳細・プログラム設計で必要な対象を一括で決定し、以降の Step では一切ユーザーへの確認を行わない。
> **「機能一覧」を単独で選択した場合（詳細設計またはプログラム設計が未選択の場合）はこのセクションをスキップして Step 3 へ進む。**
...
「全グループ」を選択した場合は `target_group_ids = ""`（空文字・空リスト）として明示的に設定する。スクリプト呼び出し時に `--group-ids` 引数を省略することで全件対象となる。
...
（このステップでの参照先: output_dir → `{output_dir}/02_詳細設計`、tmp_dir → `{output_dir}/02_詳細設計/.tmp`）

### 対象グループの選択

**詳細設計＋プログラム設計の両方選択時**: `target_group_ids_2 = target_group_ids`（Step 0-3 確定済み）。選択をスキップして委譲へ進む。

**単独実行の場合**:
- AskUserQuestion で選択する（全グループ / グループIDを指定 / コンポーネントを指定）
- feature_groups.yml がない場合は group_features.py を実行してから表示する

「グループIDを指定」の場合は Step 0-3 と同じグループ解決スクリプトで FG-XXX に変換して `target_group_ids_2` とする。
...
（このステップでの参照先: output_dir → `{output_dir}/03_プログラム設計`、tmp_dir → `{output_dir}/03_プログラム設計/.tmp`）

### 機能リスト読み込み

`/sf-memory` cat4 が生成した `docs/.sf/feature_list.json` を tmp にコピーして使う。再スキャンは行わない（二重実行・差分防止）。
...
**詳細設計＋プログラム設計の両方選択時**: Step 0-3 で確定した `target_group_ids` に属する機能IDを自動抽出する。AskUserQuestion は出さない。
```bash
python -c "
import yaml, pathlib
with open(r'{project_dir}/docs/.sf/feature_groups.yml', encoding='utf-8') as f:
...
> Step 2 を実行していない場合（プログラム設計を単独で選択した場合）も同様に「全機能」か「その他指定」の2択。
...
## Step 3: 機能一覧（実行条件を必ず確認すること）

> **実行条件**: 以下のいずれかの場合のみ実行する:
> 1. 「機能一覧」のみ選択（詳細設計・プログラム設計は未選択）
> 2. 「詳細設計+機能一覧」選択（Step 1 完了後）
>
> **実行しない場合**: 「プログラム設計+機能一覧」または「詳細+プログラム+機能一覧」選択時 → Step 2 の sf-design-writer が機能一覧.xlsx を生成するため、このステップはスキップする。

「機能一覧」を単独で選択した場合は Step 0-2 の共通情報取得後に以下を実行する。
「詳細設計+機能一覧」の場合は Step 1 完了後に実行する。

### feature_list.json の読み込み
...
読み込んだ `docs/.sf/feature_list.json` を Read ツールで読み込み、内容を `feature_list` として保持する。

### sf-design-writer に委譲（機能一覧モード）
...
sf-design-writer は `{output_dir}/01_基本設計/機能一覧.xlsx` を生成して終了する。
...
- コンポーネント名（API名・F-XXX）で対象を指定した場合は、スクリプトで対応する FG-XXX に変換してから処理する
