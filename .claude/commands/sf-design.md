...
> **「機能一覧のみ」のみ選択した場合はこのセクションをスキップして Step 3 へ進む。**
...
## Step 3: 機能一覧（「機能一覧のみ」単独選択時、またはプログラム設計完了後）

「機能一覧のみ」を選択した場合は Step 0-2 の共通情報取得後に以下を実行する。
...
### sf-design-writer に委譲（機能一覧のみモード）
...
mode:             機能一覧のみ（プログラム設計書は生成しない）
```

sf-design-writer は `{output_dir}/01_基本設計/機能一覧.xlsx` のみを生成して終了する。
