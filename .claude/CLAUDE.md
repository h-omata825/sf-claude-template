...
| 設計書・要件定義書 | salesforce-architect | **reviewer** | 整合性・スコープ・受入基準の明確性・依頼との一致 |
...
| 議事録・タスク・計画・報告書 | project-manager | **reviewer** | 決定事項の漏れ・担当者と期限・実行可能性・優先順位 |
...
- `docs/changelog.md` に変更サマリを1行追記する（日付・変更内容・関連課題ID）
...
**記録するタイミング:**
- `/backlog` `/backlog-report` で対応方針を確定し、実装を完了したとき
