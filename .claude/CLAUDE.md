...
#### 「議事録まとめて」「会議メモを保存して」
1. 生のメモを `docs/minutes/YYYY-MM-DD_{会議名}.md` に保存
2. 以下の形式で整理する:
   ```
   ## 決定事項
   ## アクションアイテム（担当・期限）
   ...
   ## 次回確認事項
   ```
3. 要件・ビジネスルールの変更・追加が含まれていれば自動で検出し提示:
   「以下の内容は要件定義書への反映が必要と思われます。`/sf-analyze` で更新しますか？」
4. スコープ変更（追加・削除・変更）が含まれていれば特に強調して提示

#### 「議事録から要件を更新して」
1. 指定された議事録ファイル（または直近の `docs/minutes/` ファイル）を読み込む
2. `docs/requirements/requirements.md` の既存要件と照合:
   - **新規要件**: FR-XXX / NFR-XXX / BR-XXX の番号を採番して追加
   - **変更要件**: 既存番号を維持しつつ内容を更新（変更前をコメントで残す）
   - **削除要件**: 削除せず「廃止」ステータスに変更（番号の連続性を維持）
3. `docs/changelog.md` に変更内容を記録
4. 変更点の一覧をユーザーに提示して確認を取ってから保存
