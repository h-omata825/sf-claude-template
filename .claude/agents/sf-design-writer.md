...
- 別クラス・別メソッドを呼び出す場合は `calls: { "text": "ClassName.method" }` で呼び出し先を明示する（フロー図で右側に紫の箱が出る）。**text は20文字以内**に収める。長いクラス名は略称を使う（例: `CommonAuthCallout.get`）
...
- [ ] **calls の形式と長さ**: `calls` を設定した場合は `{ "text": "ClassName.method" }` 形式になっているか。**text は20文字以内**か（超える場合は略称に修正する）
