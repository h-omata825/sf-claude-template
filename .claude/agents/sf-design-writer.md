...
| 各 `steps[].title` | そのステップが何をする処理か（日本語・15文字以内を目安） |
...
> ⚠️ **このエージェントが担当する種別**: Apex / Batch / Flow（非画面）/ Integration / Aura のみ。
> LWC・画面フローは **sf-screen-writer** が担当する。誤って担当してはならない。
...
| Aura | `"Aura"` | generate_feature_design.py | 設計書テンプレート.xlsx |
...
```
【Q1】このステップは条件分岐・判定処理か？（if / switch / 成否確認 / Decisionノード）
  YES →  node_type: "decision"
         branch: { "text": "エラー/NGの結果", "node_type": "error"|"success", "label": "False" }
         main_label: "True"  （省略時は自動付与）
  NO  ↓

【Q2】このステップは別クラス・別メソッドを呼び出すか？（ユーティリティ / 外部API / サブフロー）
  YES →  calls: { "text": "ClassName.method" }  （20文字以内。object_ref / branch との併用不可）
  NO  ↓

【Q3】このステップはSOQL / DML / レコード操作を実行するか？
  YES →  object_ref: { "text": "ObjectApiName" }  （FROM句 or 対象オブジェクトのAPI名）
...
```json
{
  ...
  "_overview_type_note": "通常モード: string / スケルトンモード: { purpose, trigger, preconditions, summary } の dict。スケルトンモードでは overview フィールドを dict として記述し、_parser_meta を削除してから Phase 2 へ進む。",
...
- [ ] **type フィールドの正確性**: このエージェントが扱うのは Apex/Batch/Flow/Integration のみ。LWC/画面フロー が混在していれば sf-screen-writer に委ねる
