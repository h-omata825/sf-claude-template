...
設計書テンプレートは固定パスに配置済み（毎回生成不要）:
```
c:\ClaudeCode\scripts\python\sf-doc-mcp\設計書テンプレート.xlsx
```
...
python -c "
import pathlib
t = pathlib.Path(r'c:\ClaudeCode\scripts\python\sf-doc-mcp\設計書テンプレート.xlsx')
if not t.exists():
    import subprocess
    subprocess.run(['python', r'c:\ClaudeCode\scripts\python\sf-doc-mcp\build_template.py', '--output', str(t)], check=True)
    print('テンプレート生成完了')
else:
    print('テンプレート既存: ' + str(t))
"
```
...
python c:\ClaudeCode\scripts\python\sf-doc-mcp\generate_feature_design.py \
  ...
  --template "c:\ClaudeCode\scripts\python\sf-doc-mcp\設計書テンプレート.xlsx" \
