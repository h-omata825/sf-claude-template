...
| 種別 | Phase 1 JSON フォーマット | Phase 2 スクリプト | テンプレート |
|---|---|---|---|
| Apex / Batch / Schedule | 機能設計書 JSON（steps/input_params/output_params） | generate_feature_design.py | 設計書テンプレート.xlsx |
| Flow（非画面フロー） | 機能設計書 JSON | generate_feature_design.py | 設計書テンプレート.xlsx |
| LWC | **画面設計書 JSON**（usecases/items/param_sections） | **generate_screen_design.py** | **画面設計書テンプレート.xlsx** |
| 画面フロー（Screen Flow） | **画面設計書 JSON** | **generate_screen_design.py** | **画面設計書テンプレート.xlsx** |
| Integration | 機能設計書 JSON | generate_feature_design.py | 設計書テンプレート.xlsx |

> **「画面フロー」の見分け方**: flow-meta.xml の `<processType>Flow</processType>` かつ `<Screen>` ノードを含むものが対象。`<processType>AutoLaunchedFlow</processType>` は非画面フローとして機能設計書 JSON を使う。
