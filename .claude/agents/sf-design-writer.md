...
| **LWC モーダル** | `absorb_into` に指定された親LWC | モーダルのUI・イベント処理 → 親の `usecases` に「{モーダル名}を開く」ユースケースとして追加。入出力プロパティ → 親の `param_sections` に追記 |
...
- 別クラス・別メソッドを呼び出す場合は `calls: { "text": "ClassName.methodName" }` で呼び出し先を明示する（フロー図で右側に緑の箱が出る）
- `object_ref` と `calls` は同一ステップで同時使用不可。`branch` と `calls` も同時使用不可
...
- サブフロー呼び出しは `calls: { "text": "SubFlowApiName" }` で明示する
...
```json
{
  ...
  "_calls_guide": "別クラス・別メソッドを呼び出すステップには calls を付与する。フロー図でステップの右側に緑の箱が矢印で表示され、呼び出し先を一目で把握できる。text にクラス名.メソッド名（例: EstimateHelper.createEstimate）を入れる。object_ref・branch との同時使用は不可。",
  "input_params": [
...
## Phase 1.5: 生成 JSON のセルフレビュー（全件・スクリプト実行前に必ず実施）

全 JSON を生成し終えたら、**スクリプトを呼ぶ前に**以下のチェックリストを全件に対して確認する。
問題を発見したらその場で JSON を修正してから Phase 2 へ進む。追加のツール呼び出しは不要（生成した JSON を頭の中で確認するだけ）。

### チェックリスト

**【フロー図関連】** ← 過去に繰り返し指摘されたもの
- [ ] **object_ref 漏れ**: `detail` や `sub_steps` に SELECT / INSERT / UPDATE / DELETE / UPSERT / 保存 / 更新 / 削除 / 登録 のキーワードがあるステップ全てに `object_ref` が設定されているか
  - 設定例: `"object_ref": { "text": "Account" }`
  - よくある漏れ: 「〇〇を保存する」「〇〇を更新する」という title なのに object_ref がない
- [ ] **object_ref / calls / branch の重複**: 同一ステップに複数が設定されていないか（同列に描画されるため重なる）
- [ ] **node_type: "object" の使用禁止**: `node_type` が `"object"` になっているステップがないか（`"process"` + `object_ref` に統一）
- [ ] **calls の形式**: `calls` を設定した場合は `{ "text": "ClassName.method" }` 形式になっているか。文字列直書きでも動作するが dict 形式を推奨

**【JSON 品質】**
- [ ] **抽象的タイトル禁止**: 「処理を実行」「データを取得」「処理する」のような意味のないタイトルがないか
- [ ] **detail にコード混入禁止**: `detail` に Apex コード・SOQL 構文が直接書かれていないか（コードは sub_steps に、detail は日本語説明のみ）
- [ ] **type フィールドの正確性**: LWC なのに `"Apex"`、画面フローなのに `"Flow"` になっていないか（generate_screen_design.py が必要な種別を generate_feature_design.py に渡すとエラー終了する）
- [ ] **overview の品質**: 「XXXコントローラー」「XXXコンポーネント」のような種別名のみになっていないか。具体的なオブジェクト名・処理内容・連携先が含まれているか

### 修正の進め方

問題を発見した場合:
1. 何件・どのコンポーネントのどのフィールドに問題があるか明示する
2. JSON を修正する
3. 修正後に「チェック完了・{n}件修正」と記録して Phase 2 へ進む

問題がなかった場合: 「チェック完了・問題なし」と記録して Phase 2 へ進む。
