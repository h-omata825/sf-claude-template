...
> **理由**: `generate_feature_design.py` は `steps[]` キーを必須とし、`usecases[]` キーを認識しない。LWC・画面フロー JSON は `usecases[]` 構造のため、スクリプトが `steps` を見つけられず空シートのまま終了する。逆に `generate_screen_design.py` に Apex JSON を渡しても `usecases` がなくエラーになる。
...
- [ ] **type フィールドの正確性**: LWC/画面フローに generate_feature_design.py を使うとエラー終了する
