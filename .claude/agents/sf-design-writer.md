...
3.5. overview dict を string に変換する（必須）
     スケルトンの overview は {"purpose": "", "trigger": "", "preconditions": "", "summary": ""} の dict 形式だが、
     tmp_dir に保存する design JSON では overview は string でなければならない。
     summary フィールドを記述した後、overview フィールド全体を summary の文字列に置き換えること:
       before: "overview": {"purpose": "...", "trigger": "...", "preconditions": "...", "summary": "処理概要テキスト"}
       after:  "overview": "処理概要テキスト"
     ※ generate_feature_list.py は overview を plain string として読み込むため、dict のまま保存すると機能一覧の処理概要欄が空になる
4. name / params を記述する
