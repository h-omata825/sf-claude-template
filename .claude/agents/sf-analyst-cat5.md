...
6. **孤立コンポーネントを見落とさない**: どのFGにも割り当てられなかったコンポーネントを `GRP-CMN（共通基盤）` に全量格納し、完了報告に「孤立コンポーネント候補」として列挙する。
...
> **コード読み取り失敗時のフォールバック**: ファイルが存在しない・構文エラー・取得不能の場合は `operated_objects: []` として記録し、割り当て結果を `**[要確認]**` コメント付きで `GRP-CMN（共通基盤）` に仮置きする。完了報告の「要確認事項」に列挙する。
...
| 5 | **対応なし**: どのUCにも対応付けられない → `GRP-CMN（共通基盤）` に割り当て（孤立コンポーネント候補として記録） |
...
- `GRP-CMN（共通基盤）` への割り当てに疑問がある場合は `# **[推定]**` コメントを付ける
...
- **孤立コンポーネントの確認**: GRP-CMN（共通基盤）に集まりすぎた場合（全体の30%超）は分類精度を疑う。以下の手順で再調査する:
  1. GRP-CMN（共通基盤）内の各コンポーネントの `operated_objects` を再確認する
  2. usecases.md の全UC の `related_objects` と突き合わせて部分一致があるか確認する
  3. `docs/design/` 配下の設計書の「関連UC」フィールドを参照して割り当てヒントを得る
  4. それでも対応付けられないものだけを GRP-CMN（共通基盤）に残す
...
### 孤立コンポーネント候補（GRP-CMN（共通基盤）に割り当て）
