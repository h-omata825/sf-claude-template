...
> **必須**: このフェーズの対象グループ確認は **必ず AskUserQuestion で行う**。以下を厳守:
> - グループ一覧（`FG-XXX: name_ja (Nコンポーネント)` の行・件数内訳）を **assistant メッセージ（地の文）に出力しない**。
> - 件数表示が必要な場合は AskUserQuestion の label / description 内（`{n}` 等の補間）でのみ行う。
> - 下の Python スクリプトの stdout（グループ一覧の `print(...)` 出力）は **件数把握のための内部値**であり、ユーザーへ見せるためのものではない。
> - 「対象グループの選択をお願いします」のようなテキスト確認は禁止。AskUserQuestion を直接呼ぶこと。
...
グループ一覧の件数取得（以下の Python 出力は内部用。stdout の内容を assistant メッセージに転記しないこと）:
```bash
python -c "
import yaml
