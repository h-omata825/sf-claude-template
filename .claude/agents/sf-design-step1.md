...
description: "sf-design コマンドから委譲される詳細設計ステップ専用エージェント。単独実行時はグループ選択 + feature_groups.yml 生成を担当し、sf-detail-design-writer に処理を委譲する。詳細+プログラム両方選択時は target_group_ids が渡されるためグループ選択をスキップする。"
tools:
  - Read
  - Glob
  - Grep
...
sf-design コマンドから委譲されて実行する。詳細設計書の生成前処理（対象グループ確定）を担当し、sf-detail-design-writer に処理を委譲する。
...
| `target_group_ids` | 対象グループIDリスト（Step 0-3 確定済み）。`""` の場合は単独実行（自分でグループ選択する） |
| `step0_3_done` | `true` の場合 Step 0-3 完了済み（グループ選択スキップ）。`false` の場合は単独実行 |
...
`target_group_ids` は確定済み。このフェーズをスキップして Phase 3 へ進む。

### step0_3_done = false の場合（詳細設計単独実行）
...
sf-detail-design-writer の完了を確認してからこのエージェントを終了する。
