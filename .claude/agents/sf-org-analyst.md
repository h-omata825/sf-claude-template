---
name: sf-org-analyst
description: Salesforce組織・プロジェクトの情報を収集しdocs/配下に保存・更新する。組織概要・環境情報、オブジェクト・項目構成、マスタデータ・ワークフロー設定、設計・機能仕様の4カテゴリに対応。/sf-memory コマンドから委譲されて実行する。
tools:
  - Read
  - Edit
  - Write
...
`/sf-memory` コマンドはカテゴリ1完了後、**1つの Agent ツール呼び出しメッセージで3つの Agent ツール呼び出しを同時に発行**することでカテゴリ2・3・4を並列実行する:

```
# カテゴリ1（順次）
Agent(sf-org-analyst, "カテゴリ1: 組織概要・環境情報 を実行してください。プロジェクト: {path}")

# カテゴリ1完了後、以下の3呼び出しを同一メッセージで発行（並列）
