...
`docs/logs/decisions.md` に判断記録を追記する:
...
デプロイ実施・仕様変更・オブジェクト変更が発生した場合は、完了報告の末尾に以下を付記する:

```
【ドキュメント更新推奨】
① Apex / Flow / LWC / Trigger を変更した場合 → /sf-memory カテゴリ4
   対象コンポーネント: {変更したコンポーネント名一覧}
   完了後: 影響するFG（{FG名}）の詳細設計・プログラム設計書の再生成を推奨

② オブジェクト項目 / レイアウト / レコードタイプ / 入力規則を変更した場合 → /sf-memory カテゴリ2
   対象オブジェクト: {変更したオブジェクト名一覧}

③ 業務仕様・要件が変わった場合 → /sf-memory カテゴリ1（requirements.md 更新）
```

コードのみのバグ修正（仕様変更・デプロイなし）の場合はこのステップをスキップしてよい。
