...
開始: {Phase 0 で記録した開始時刻}
終了: {現在時刻}
経過: {X時間Y分}

途中で休憩・中断を挟んだ場合は時間を教えてください（例: 「昼食1時間」「休憩30分×2」）。
入力がなければ経過時間がそのまま実績工数になります。
```

実績工数 = 経過時間 - ブレーク時間 で計算する。

xlsx を作成している場合:
- 「サマリー・経緯」シートの「対応完了日時」「実績工数（CC使用）」を更新
- `docs/logs/effort-log.md` の該当行に実績工数を追記する
...
### 7. ドキュメント更新通知（デプロイ・仕様変更・組織変更を伴う場合）
