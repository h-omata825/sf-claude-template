...
`.claude/CLAUDE.md`「実装後のドキュメント更新」ルールに従い、変更内容に応じて更新する（`.claude/CLAUDE.md` が存在しない場合は changelog.md のみ更新して次の変更ファイルへ進む）:
...
2. 「特に確認したい点」を 1〜3 個テキストで挙げる（典型例: 「実装中に計画書に記載のなかった構造 X を発見したため採用アプローチを変えた」「implementation-plan.md の改版が必要な箇所の確認」）。確認事項がない場合は「特に確認事項はありませんでした」と明記する
