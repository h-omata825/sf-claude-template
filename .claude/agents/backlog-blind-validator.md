...
5. `approach-plan.md` の採用方針テキスト（対応方針のみ・実装計画は含まない）
...
parent から渡された investigation.md・approach-plan.md のテキストを使用する。
**いずれかの内容が空または欠落している場合は「必要情報が渡されていません: {項目名}」を parent に返し、実装案生成を中断する。**
...
**完了基準**: Step 2 で設計した処理構造・データ設計・SOQL・エラーハンドリング・副作用対応の全5観点について主要ポイントが記載されたら完了とする。
