...
`docs/logs/{issueID}/investigation.md` のテストシナリオと再現条件を読む。種別値（`バグ` / `追加要望` / `その他`）は呼び出し元から渡されたパラメータを使用する（investigation.md の種別欄と食い違いがある場合はユーザに確認してから後続の Step 4.5 の分岐判定に使用する）。
...
必ず **Sandbox の alias** を使用する。alias が未確認なら「Sandbox の alias を教えてください」とテキストで確認してから実行する。`sf org display --target-org <alias> --json` で `isSandbox=true` を確認できない・本番接続が確認された場合は Apex テストを実行せず、ユーザに Sandbox 移行を案内する（本番組織でのテスト実行はガバナ消費・テストデータ生成等の副作用があるため禁止）。
...
Phase 1.7 で確認した再現手順を**修正後の Sandbox** で再実行し、**事象が発生しないこと**を確認:
- investigation.md の「再現条件」記載の前提データ・操作ユーザ・操作手順を忠実に再現
- スクリーンショット・ログを `docs/logs/{issueID}/evidence/after/repro/` に保存
- Phase 1.7 で取得した Before（`evidence/before/repro/`）と対比可能な状態にする
...
4.5-A / 4.5-C はスキップ可。従来のテスト観点のみ適用。
...
Phase 1.7 で取得した Before エビデンスと対になる After エビデンスを取得する:
...
- **テストを止める**
- 発見した内容を具体的に説明する
- 対応案（Phase 4 差し戻し or 環境準備依頼）をユーザに提示して確認を取ってから再開する
...
- [ ] バグの場合、影響範囲チェックリスト（4.5-B）の4観点を全て確認したか
...
2. 「特に確認したい点」を 1〜3 個テキストで挙げる（典型例: 「ユーザ合同確認が取れていないシナリオ X の扱い」「Before/After エビデンスの Before が実装前に取得されているか」）
