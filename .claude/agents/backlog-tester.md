...
`docs/logs/{issueID}/investigation.md` のテストシナリオと再現条件を読む。種別値（`バグ` / `追加要望` / `その他`）は呼び出し元から渡されたパラメータを使用する（investigation.md の種別欄と食い違いがある場合はユーザに確認してから後続の Step 4.5 の分岐判定に使用する。無応答時・食い違いが解消できない場合は呼び出し元パラメータの種別を優先して続行する）。
`docs/logs/{issueID}/implementation-plan.md` の実装方針まとめを読む。

**いずれかが存在しない場合**: investigation.md が無ければ `/backlog Phase 1（investigator）`、implementation-plan.md が無ければ `/backlog Phase 3（backlog-planner Phase B）` から先に実施するようユーザに案内し、tester の処理を中止する。
...
必ず **Sandbox の alias** を使用する。alias が未確認なら「Sandbox の alias を教えてください」とテキストで確認してから実行する。`sf org display --target-org <alias> --json` で `isSandbox=true` を確認できない・本番接続が確認された場合は Apex テストを実行せず、ユーザに Sandbox 移行を案内する（本番組織でのテスト実行はガバナ消費・テストデータ生成等の副作用があるため禁止）。コマンド自体が失敗した場合（CLI 未認証・ネットワーク断等）も同様に Apex テストを実行せず、エラー内容をユーザに伝えて Sandbox 認証を確認してから再実行する。
...
python scripts/python/backlog-xlsx/update_records.py \
  ...
  --value "PASS"
# テストケースが複数ある場合は --row を 8, 9, ... と 1 ずつ増やして繰り返す（ベース行 7 + テストケース番号 - 1）
```
