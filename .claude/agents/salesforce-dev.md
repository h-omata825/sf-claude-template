...
3. **データ型選定**: 用途に応じて最適なデータ型を選定。迷う場合は選択肢と理由を提示して確認
...
**関連設定の確認（自動変更しない、必ず確認を取る）:**

| 設定 | 確認方法 |
|---|---|
| FLS（項目レベルセキュリティ） | 「どの権限セット/プロファイルにFLSを設定しますか？（全て / 指定 / 後で手動設定）」 |
...
`docs/catalog/` で対象オブジェクトの自動化を確認し、衝突リスクを提示する:
- 同じオブジェクト・同じタイミングの既存レコードトリガーフロー（実行順序は制御可能だが複雑化する）
- ワークフロールール / プロセスビルダーの残存（レガシー自動化との共存リスク）
- Apexトリガーとの実行順序（Apexトリガー → フロー の順）
- 入力規則との条件重複・矛盾
