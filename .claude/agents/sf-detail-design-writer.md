...
**主語欠落の断片を書かない。書けないなら空文字列にする。**

| NG（断片） | OK（完全文） |
|---|---|
| `は、ため動作不全` | 「標準ボイラープレート VF。動作は Experience Cloud の標準設定に依存する」 |
| `一致すれば または遷移` | 「ユーザー名照合が一致した場合に申込画面に遷移し、不一致の場合はエラー画面に遷移する」 |
| `でパスワード設定 → 遷移` | 「新しいパスワードを入力・確認して保存ボタンを押下すると、完了画面に遷移する」 |

断片を書かず、以下の形式で書く:
- **「〇〇コンポーネントは〜を担当する」** （主語省略可、ただし述語まで完結させること）
- 技術識別子は業務用語に置き換える（上の「禁止」セクション参照）
...
- アクターは「お客様」「GF社担当者」「管理部門」「承認者」「自動フロー」等の業務上の登場人物。コンポーネント名を入れない
- ステップは3〜5件。起点（誰が何をきっかけにするか）→ システム処理（自動フロー）→ 最終結果（誰が何を受け取るか）の順

**action / label のペア例**:

| action（処理内容欄） | label（図形ラベル） |
|---|---|
| 取引先責任者情報を確認し、ユーザー発行画面から発行を申請する | ユーザー発行を申請 |
| ユーザー作成・仮パスワード発行・メール送信を実行する | ユーザー作成・通知 |
| 初期パスワードメールを受信し、Experience Cloudポータルにアクセスする | ポータルへアクセス |
| パスワードリセット画面でユーザ名／新パスワードを入力し、登録ボタンを押下する | パスワード再設定 |
| 契約申込 URL パラメータから対象レコードを特定し、取引先責任者・ユーザーの照合を行う | 契約申込を特定 |

**情報源の優先順位**:
1. 基本設計 JSON の `business_flow` がある場合 → それを**そのまま流用**（表現だけ詳細設計向けに微調整してよい）
2. ない場合は `screens[]` + `processing_purpose` + `prerequisites` から組み立てる
3. `data_flow_overview` の矢印トークンは**Step1のアクション文として直接使わない**（技術フローの表現であり業務アクションではない）

**良い例（ユーザー発行機能）**:
- Step1: GF社担当者 / 「取引先責任者の情報を確認し、ユーザー発行画面から発行を申請する」
- Step2: 自動フロー / 「ユーザー作成・仮パスワード発行・メール送信を実行する」
- Step3: お客様 / 「初期パスワードメールを受信し、Experience Cloudポータルにアクセスする」
