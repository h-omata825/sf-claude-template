...
description: "LWC・画面フロー専用の画面設計書（Excel）生成エージェント。sf-doc コマンドの Step D から委譲されて実行する。usecases[] 構造の画面設計書 JSON を生成し generate_screen_design.py で Excel に変換する。Apex・Flow・Batch は対象外（sf-design-writer が担当）。"
---
...
`/sf-doc` コマンドの Step D のうち **LWC・画面フロー** を担当する専門エージェント。
...
| `feature_list` | scan_features.py の出力（LWC・画面フロー のみ抽出済み） |
...
> **一時ファイルの作成場所**: 処理中に一時的な Python スクリプトや JSON ファイルを作成する場合は、必ず `{tmp_dir}` 配下に置くこと。
...
- [ ] **type フィールドの正確性**: `"LWC"` または `"画面フロー"` になっているか
...
```
✅ 画面設計書.xlsx — {件数}ファイル（LWC: X件 / 画面フロー: Y件）
出力先: {output_dir}
```
