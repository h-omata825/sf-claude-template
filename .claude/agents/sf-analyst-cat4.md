...
### 設計書再生成推奨（更新があった場合のみ）
更新したコンポーネントが属するFGを feature_groups.yml から特定し、以下を通知する:

【設計書の再生成を推奨します】
影響FG: {FG-xxx: FG名} ほかXX件
→ /sf-design で以下を再生成してください:
  - 詳細設計書（sf-detail-design-writer）対象: {FG名一覧}
  - プログラム設計書（sf-design-writer）対象: {コンポーネント名一覧}
```
